...
async function main() {
  const fileArg = process.argv[2] || 'data/companies.json';
  ...
  try {
    ...
    for (const r of rows) {
      ...
      // Skip deleted/inactive companies (status = '2' or status = 2)
      if (status === '2' || status === 2) {
        console.log(`   Skipping deleted company: ${name} (ID: ${id})`);
        continue;
      }
      
      if (!id || !name) continue;
      await client.query(
        `INSERT INTO companies (id, name, img) VALUES ($1,$2,$3)
    ...
    console.log(`Imported companies: ${rows.length}`);
  } catch (e) {
    await client.query('ROLLBACK');
    console.error('Companies import failed:', e.message);
    process.exitCode = 1;
  } finally {
    client.release();
    await pool.end();
