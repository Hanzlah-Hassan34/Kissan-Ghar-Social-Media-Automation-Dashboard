-- ============================================
-- COMPLETE SCHEMA FOR KISSAN GHAR DASHBOARD
...
CREATE TABLE generatedVideos (
    ...
    script_status status DEFAULT 'pending',
    script_generated_count INT DEFAULT 0,
    created_at TIMESTAMP WITH TIME ZONE DEFAULT now(),
    updated_at TIMESTAMP WITH TIME ZONE DEFAULT now()
);
...
CREATE TABLE publishedVideo (
    ...
    description TEXT,
    description_status status DEFAULT 'pending',
    tags TEXT,
    tags_status status DEFAULT 'pending',
    tags_generated_count INT DEFAULT 0,
    title_generated_count INT DEFAULT 0,
    description_generated_count INT DEFAULT 0,
...
-- 6. Video References (many-to-many)
...
-- 10. Update product images to use full URL
UPDATE products
